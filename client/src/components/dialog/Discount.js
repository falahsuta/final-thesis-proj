...
export default (props) => {
  ...
  const fetchActivate = async () => {
    let url = `http://localhost:8080/discountspaginate?limit=3&page=${currPage}`
  ...
  const kanan = () => {
    // if (Math.ceil(topup.total_counts / 3) != currPage) {
    ...
    <>
      {topup && topup.discounts && topup.discounts.length > 0 && (
          <Grid container spacing={2} direction="row" justify="center" alignItems="center" style={{margin: "10px"}}>
            <Grid item xs={4} md={4}>
              <Info useStyles={useD01InfoStyles} style={{marginLeft: "30px"}}>
                <Typography color="textPrimary" variant="h5" component="h2">
                  {"Your Discount".toUpperCase()}
                </Typography>
                {spacing("10px")}
              </Info>
            </Grid>
            <Grid item xs={4} md={4}>
              <div style={{marginLeft: "-40px"}}>
                <Picks items={topup["discounts"]} markProps={markProps} discount={true} />
              </div>
            </Grid>
            <Grid item xs={3} md={3}>
